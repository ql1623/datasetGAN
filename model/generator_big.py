...
class FusionBlock(nn.Module):
    def __init__(self, in_channels, out_channels, groups, initial=True, act_fn="leaky", norm="batch"):
        ...
        self.group_conv =  nn.Sequential(
            nn.Conv3d(in_channels, in_channels, kernel_size=(2, 3, 3), padding=(0, 1, 1), groups=groups),
            ...
            nn.BatchNorm3d(in_channels) if norm=="batch" else nn.InstanceNorm3d(in_channels),
            nn.LeakyReLU(0.2, inplace=True) if act_fn=="leaky" else nn.ReLU(inplace=True),
        )
        self.merge_conv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1) if initial 
            else nn.Conv2d(in_channels*2, out_channels, kernel_size=3, stride=1, padding=1),
    ...
    def forward(self, x1, x2, prev_fusion=None):
        ...
        if prev_fusion is not None:
            x = torch.cat([x, prev_fusion], dim=1)
            x = self.merge_conv(x)
        return x
...
class datasetGAN(nn.Module):
    ...
    def __init__(self,input_channels, output_channels, ngf): # input_channels = 1, output_channel = 1, ngf = 64/32 , features = [16,32,64,128]
        ...
        # decoding path
        self.up1_mod1     = SamplingBlock(self.features*8*2, self.features*8, down=False, act_fn="relu", norm="batch", use_dropout=False)
        ...
        # decoding path
        self.up1_mod2     = SamplingBlock(self.features*8*2, self.features*8, down=False, act_fn="relu", norm="batch", use_dropout=False)
        ...
        # --------------- fusion network ---------------
        # encoding path
        self.fusion1 = FusionBlock(self.features, self.features*2, groups=self.features, initial=True, act_fn="leaky", norm="batch")
        self.fusion_down1 = SamplingBlock(self.features*2, self.features*2, down=True, act_fn="leaky", norm="batch", use_dropout=False)
        
        self.fusion2 = FusionBlock(self.features*2, self.features*4, groups=self.features*2, initial=False, act_fn="leaky", norm="batch")
        self.fusion_down2 = SamplingBlock(self.features*4, self.features*4, down=True, act_fn="leaky", norm="batch", use_dropout=False)
        
        self.fusion3 = FusionBlock(self.features*4, self.features*8, groups=self.features*4, initial=False, act_fn="leaky", norm="batch")
        self.fusion_down3 = SamplingBlock(self.features*8, self.features*8, down=True, act_fn="leaky", norm="batch", use_dropout=False)
        
        self.fusion4 = FusionBlock(self.features*8, self.features*8, groups=self.features*8, initial=False, act_fn="leaky", norm="batch")
        # self.fusion_down4 = SamplingBlock(self.features*8, self.features*8, down=False, act_fn="relu", norm="batch", use_dropout=False)
        
        # bottleneck
        self.bottleneck_fuse_layer = nn.Sequential(
            nn.Conv2d(self.features*8, self.features*8, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True)
        )
        
        # decoding path
        self.fusion5 = FusionBlock(self.features*8, self.features*4, groups=self.features*8, initial=False, act_fn="relu", norm="batch")
        self.fusion_up1 = SamplingBlock(self.features*4, self.features*4, down=False, act_fn="relu", norm="batch", use_dropout=False)
        
        self.fusion6 = FusionBlock(self.features*4, self.features*2, groups=self.features*4, initial=False, act_fn="relu", norm="batch")
        self.fusion_up2 = SamplingBlock(self.features*2, self.features*2, down=False, act_fn="relu", norm="batch", use_dropout=False)
        
        self.fusion7 = FusionBlock(self.features*2, self.features*1, groups=self.features*2, initial=False, act_fn="relu", norm="batch")
        self.fusion_up3 = SamplingBlock(self.features*1, self.features*1, down=False, act_fn="relu", norm="batch", use_dropout=False)
        
        self.fusion8 = FusionBlock(self.features*1, self.features*1, groups=self.features*1, initial=False, act_fn="relu", norm="batch")
        self.fusion_up4 = SamplingBlock(self.features*1, self.features*1, down=False, act_fn="relu", norm="batch", use_dropout=False)
    ...
    def forward(self,inputs):
        ...
        # ----- fusion part (for encoding path) -----
        fusion_1 = self.fusion1(feat_down_fusion_1[0], feat_down_fusion_2[0], None)
        fusion_d1 = self.fusion_down1(fusion_1)
        
        fusion_2 = self.fusion2(feat_down_fusion_1[1], feat_down_fusion_2[1], fusion_d1)
        fusion_d2 = self.fusion_down2(fusion_2)
        
        fusion_3 = self.fusion3(feat_down_fusion_1[2], feat_down_fusion_2[2], fusion_d2)
        fusion_d3 = self.fusion_down3(fusion_3)
        
        fusion_4 = self.fusion4(feat_down_fusion_1[3], feat_down_fusion_2[3], fusion_d3)
        bottleneck_fusion = self.bottleneck_fuse_layer(fusion_4)
        ...
        # feat_down_fusion_1_rev = feat_down_fusion_1[::-1]
        # feat_down_fusion_2_rev = feat_down_fusion_2[::-1]
        fusion_5 = self.fusion5(feat_down_fusion_1[3], feat_down_fusion_2[3], bottleneck_fusion)
        fusion_upsamp1 = self.fusion_up1(fusion_5)
        
        fusion_6 = self.fusion6(feat_down_fusion_1[2], feat_down_fusion_2[2], fusion_upsamp1)
        fusion_upsamp2 = self.fusion_up2(fusion_6)
        
        fusion_7 = self.fusion7(feat_down_fusion_1[1], feat_down_fusion_2[1], fusion_upsamp2)
        fusion_upsamp3 = self.fusion_up3(fusion_7)
        
        fusion_8 = self.fusion8(feat_down_fusion_1[0], feat_down_fusion_2[0], fusion_upsamp3)
        fusion_upsamp4 = self.fusion_up4(fusion_8)
        
        out_fusion = self.fusion_final_conv(fusion_upsamp4)
...
def test():
    ...
    # x_disc = torch.randn((1,2,128,128))
