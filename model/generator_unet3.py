...
class datasetGAN(nn.Module):
    ...
    def __init__(self,input_channels, output_channels, ngf): # input_channels = 1, output_channel = 1, ngf = 64/32 , features = [16,32,64,128]
        ...
        self.bottleneck_mod1 = nn.Sequential(
            nn.Conv2d(self.features*4, self.features*4, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True)
        )
        
        # decoding path
        self.up1_mod1     = SamplingBlock(self.features*4, self.features*4, down=False, act_fn="relu", norm="batch", use_dropout=False)
        self.upconv1_mod1 = ConvBlock(self.features*4*2, self.features*4, act_fn="relu", norm="batch", use_dropout=False)
        
        self.up2_mod1     = SamplingBlock(self.features*2*2, self.features*2, down=False, act_fn="relu", norm="batch", use_dropout=False)
        self.upconv2_mod1 = ConvBlock(self.features*2*2, self.features*2, act_fn="relu", norm="batch", use_dropout=False)
        
        self.up3_mod1     = SamplingBlock(self.features*1*2, self.features*1, down=False, act_fn="relu", norm="batch", use_dropout=False)
        self.upconv3_mod1 = ConvBlock(self.features*1*2, self.features*1, act_fn="relu", norm="batch", use_dropout=False)
        ...
        # decoding path
        self.up1_mod2     = SamplingBlock(self.features*4, self.features*4, down=False, act_fn="relu", norm="batch", use_dropout=False)
        self.upconv1_mod2 = ConvBlock(self.features*4*2, self.features*4, act_fn="relu", norm="batch", use_dropout=False)
        
        self.up2_mod2     = SamplingBlock(self.features*2*2, self.features*2, down=False, act_fn="relu", norm="batch", use_dropout=False)
        self.upconv2_mod2 = ConvBlock(self.features*2*2, self.features*2, act_fn="relu", norm="batch", use_dropout=False)     
           
        self.up3_mod2     = SamplingBlock(self.features*1*2, self.features*1, down=False, act_fn="relu", norm="batch", use_dropout=False)
        self.upconv3_mod2 = ConvBlock(self.features*1*2, self.features*1, act_fn="relu", norm="batch", use_dropout=False)
        ...
        # encoding path       
        # decoding path
        self.fusion0 = FusionBlock(self.features*4, self.features*4, groups=self.features*4, act_fn="relu", norm="batch")
        self.fusion_up0 = SamplingBlock(self.features*4, self.features*4, down=False, act_fn="relu", norm="batch", use_dropout=False)
        
        self.fusion1 = FusionBlock(self.features*4, self.features*4, groups=self.features*4, act_fn="relu", norm="batch")
        self.fusion_conv1 = ConvBlock(self.features*4*2, self.features*4, act_fn="relu", norm="batch", use_dropout=False)
        self.fusion_up1 = SamplingBlock(self.features*4, self.features*2, down=False, act_fn="relu", norm="batch", use_dropout=False)
        
        self.fusion2 = FusionBlock(self.features*2, self.features*2, groups=self.features*2, act_fn="relu", norm="batch")
        self.fusion_conv2 = ConvBlock(self.features*2*2, self.features*2, act_fn="relu", norm="batch", use_dropout=False)
        self.fusion_up2 = SamplingBlock(self.features*2, self.features*1, down=False, act_fn="relu", norm="batch", use_dropout=False)
        
        self.fusion3 = FusionBlock(self.features*1, self.features*1, groups=self.features*1, act_fn="relu", norm="batch")
        self.fusion_conv3 = ConvBlock(self.features*1*2, self.features*1, act_fn="relu", norm="batch", use_dropout=False)
    ...
    def forward(self,inputs):
        ...
        upconv1_x1 = self.upconv1_mod1(torch.cat((up1_x1, skip_connections_1[2]),dim=1))
        
        up2_x1 = self.up2_mod1(upconv1_x1)
        upconv2_x1 = self.upconv2_mod1(torch.cat((up2_x1, skip_connections_1[1]),dim=1))
        
        up3_x1 = self.up3_mod1(upconv2_x1)
        upconv3_x1 = self.upconv3_mod1(torch.cat((up3_x1, skip_connections_1[0]),dim=1))
        ...
        upconv1_x2 = self.upconv1_mod2(torch.cat((up1_x2, skip_connections_2[2]),dim=1))
        
        up2_x2 = self.up2_mod2(upconv1_x2)
        upconv2_x2 = self.upconv2_mod2(torch.cat((up2_x2, skip_connections_2[1]),dim=1))
        
        up3_x2 = self.up3_mod2(upconv2_x2)
        upconv3_x2 = self.upconv3_mod2(torch.cat((up3_x2, skip_connections_2[0]),dim=1))
...
# --------------------------------------------- #
# disc of features=[32,64,128,256,512]
# Total params: 1,574,529
# Trainable params: 1,574,529
# Non-trainable params: 0
