...
"""Have Segmentation Network inside GAN, take feature layers right before out_conv of fusion"""
"""only 1 final conv in fusion, no pre_out_features"""
...
class ConditionalBlock_v2(nn.Module):
    def __init__(self, in_channels, out_channels, feat_img_size, is_separate=False): 
        ...
        Parameters:
            ...
            out_channels (int): num channels in intermediate feautures channels in model
            feat_img_size (int): height and width dims of intermediate feautures channels in model
            is_separate (bool, optional): if to make each element in one-hot as separate or 1 channel . Defaults to False.
        """
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=1, features=[32,64,128,256,512]):
        ...
        self.cond = ConditionalBlock(3, out_channels=1, feat_img_size=256, num_output_comb=3, has_act_fn=False, is_linear=False)
